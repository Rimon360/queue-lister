 ...
 * so browser.cookies can only answer while the queue window is still open —
...
function rememberCookie(tabId, cookie) {
  ...
  if (browser.storage && browser.storage.session) {
    browser.storage.session.set({ ["qc_" + tabId]: cookie }).catch(() => {})
  }
...
if (browser.webRequest.OnBeforeSendHeadersOptions && browser.webRequest.OnBeforeSendHeadersOptions.EXTRA_HEADERS) {
...
async function cookieFromStore(senderTabId, senderIsIncognito) {
  ...
  if (!store) return ""
  const cookies = await browser.cookies.getAll({ domain: "queue-it.net", storeId: store.id })
